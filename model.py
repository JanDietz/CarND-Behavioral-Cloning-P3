...
# Read the csv-file to get all image pathes and steering angles
samples = []
...
correction = 0.2  # correction value for steering angle
	...
	while 1: # Loop forever so the generator never terminates
		...
		for offset in range(0, num_samples, batch_size):
			...
			for batch_sample in batch_samples:
				...
				center_image = bgr2rgb(cv2.imread(name))              
				#name = './IMG/'+batch_sample[1].split('/')[-1]
				left_image = bgr2rgb(cv2.imread(name))
				#name = './IMG/'+batch_sample[2].split('/')[-1]
				right_image = bgr2rgb(cv2.imread(name))
